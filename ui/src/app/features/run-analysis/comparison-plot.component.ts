import { Component, effect, ElementRef, inject, input, OnChanges, OnDestroy, SimpleChanges, ViewChild } from '@angular/core';
    ...
    <section class="plot-panel" [class.maximized]="maximized()">
      <div class="axis-groups">
        <fieldset><legend>X axis</legend>
          <label>Parameter<p-select [options]="parameterOptions" [(ngModel)]="xKey" optionLabel="label" optionValue="value" (onChange)="draw()" /></label>
          <label>Scale<p-select [options]="scaleOptions" [(ngModel)]="xScale" optionLabel="label" optionValue="value" (onChange)="draw()" /></label>
        </fieldset>
        <fieldset><legend>Y axis</legend>
          <label>Metric<p-select [options]="metricOptions" [(ngModel)]="yKey" optionLabel="label" optionValue="value" (onChange)="draw()" /></label>
          <label>Scale<p-select [options]="scaleOptions" [(ngModel)]="yScale" optionLabel="label" optionValue="value" (onChange)="draw()" /></label>
        </fieldset>
      </div>
...
export class ComparisonPlotComponent implements OnChanges, OnDestroy {
  ...
  constructor() {
    effect(() => { this.theme.dark(); window.setTimeout(() => this.draw()); });
  }
  ...
  async draw(): Promise<void> {
    ...
    const pairs = this.rows().map(row => ({ x:Number(row[this.xKey]), y:Number(row[this.yKey]), series:String(row['__series'] || analysis.payload.software_name || 'Run') }))
    ...
    const traces = [...groups.entries()].map(([name, values]) => {
      ...
      return { name, x:sorted.map(value => value.x), y:sorted.map(value => value.y), type:'scatter', mode:'lines+markers', hovertemplate:`${xColumn?.label}: %{x}<br>${yColumn?.label}: %{y}<extra>${name}</extra>` };
    });
    ...
    Plotly.react(this.plotElement.nativeElement, traces, {
      ...
      paper_bgcolor:dark ? '#18201b' : '#ffffff', plot_bgcolor:dark ? '#111814' : '#fafcf9', font:{color:dark ? '#e7eee9' : '#17201b'},
      xaxis:{title:{text:xColumn?.label},type:this.xScale,automargin:true,gridcolor:dark ? '#34413a' : '#dfe5e1'},
      yaxis:{title:{text:yColumn?.label},type:this.yScale,automargin:true,gridcolor:dark ? '#34413a' : '#dfe5e1'},
