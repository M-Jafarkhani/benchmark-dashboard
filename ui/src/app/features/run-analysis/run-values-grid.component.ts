...
@Component({
  ...
  template: `
    <div class="actions"><p-button label="Export CSV" icon="pi pi-download" size="small" [outlined]="true"
      [disabled]="!gridApi" (onClick)="exportCsv()" /></div>
    <ag-grid-angular class="values-grid" [rowData]="data().rows" [columnDefs]="columnDefs()"
      [defaultColDef]="defaultColDef" (gridReady)="gridReady($event)" (filterChanged)="filterChanged($event)" />
  `,
  styles: [`
    .actions { display:flex; justify-content:flex-end; margin-bottom:.5rem; }
...
export class RunValuesGridComponent {
  ...
  readonly columnDefs = computed<ColDef[]>(() => {
    ...
    const metadata: ColDef[] = analysis.runCount > 1 ? [
    ...
    return [...metadata, ...analysis.columns.map(column => {
      const numeric = analysis.rows.every(row => row[column.key] === null || typeof row[column.key] === 'number');
  ...
  exportCsv(): void {
    const analysis = this.data();
    const name = (analysis.payload.benchmark || 'benchmark-values').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    this.gridApi?.exportDataAsCsv({ fileName: `${name || 'benchmark-values'}${analysis.runCount > 1 ? '-comparison' : ''}.csv`, exportedRows: 'filteredAndSorted' });
  }
}
